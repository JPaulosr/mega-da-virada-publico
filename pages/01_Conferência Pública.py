...
<style>
    ...
    .lottery-ball {
        ...
        box-shadow: 2px 2px 5px rgba(0,0,0,0.3); margin: 3px; border: 1px solid #ccc;
    }
    
    .ball-hit {
        background: radial-gradient(circle at 30% 30%, #4CAF50, #1B5E20);
        color: white; border: 1px solid #004D40; 
        text-shadow: 1px 1px 2px rgba(0,0,0,0.5); transform: scale(1.1); z-index: 2;
    }
    
    .ball-miss { background: #333; color: #777; border: 1px solid #444; opacity: 0.6; box-shadow: none; }

    .player-card {
        background-color: #1e1e1e; border-radius: 12px; padding: 15px; margin-bottom: 12px;
        border: 1px solid #333; box-shadow: 0 4px 6px rgba(0,0,0,0.1); position: relative; overflow: hidden;
    }
    ...
    .card-points { 
        font-size: 22px; font-weight: 900; background: #000; padding: 5px 10px; border-radius: 8px;
        min-width: 40px; text-align: center;
    }
    .card-dezenas { display: flex; flex-wrap: wrap; gap: 2px; }

    .draw-panel {
        background: #111; padding: 20px; border-radius: 16px; text-align: center; 
        margin-bottom: 20px; border: 1px solid #333; box-shadow: 0 0 20px rgba(0,0,0,0.5) inset;
    }
...
with st.sidebar:
    ...
    # --- ATUALIZADO AQUI PARA 850 MILHÕES ---
    est_sena = st.number_input("Prêmio Sena (6)", value=850000000.0, step=1000000.0, format="%.2f")
...
col_sel, col_space = st.columns([1, 0.01])
...
if len(picked) == 6:
    ...
    if bets.empty:
        ...
        for _, row in bets.iterrows():
            ...
            css_class = "card-normal"; cor_pts = "#555"; label_premio = ""
            if acertos == 6: css_class="card-sena"; cor_pts="#FFD700"; label_premio="SENA 🏆"
            elif acertos == 5: css_class="card-quina"; cor_pts="#4CAF50"; label_premio="QUINA 🥈"
            elif acertos == 4: css_class="card-quadra"; cor_pts="#2196F3"; label_premio="QUADRA 🥉"
